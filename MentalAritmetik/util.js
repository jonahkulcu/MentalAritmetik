...
function update_right_side_bar(newScore,nameOfProgress) {
    ...
    if(nameOfProgress==="finger"){
        ...
        let progressBar = document.getElementById("progress-bar-finger1");
    if(newScore>=1000){
        progressBar.style.width =100 + "%";
        progressBar.setAttribute("aria-valuenow", 100);
            progressBar.setAttribute("class", "progress-bar-finger1 bg-success");
            document.getElementById("daily-progress-finger1").innerHTML = 100;
            return;
    } 
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            document.getElementById("daily-progress-finger1").innerHTML = newScore;
